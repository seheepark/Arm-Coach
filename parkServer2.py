...
def compareVelocity1(current, first):
	if current < first:
	#	engine.say("Hurry up")
		print "Hurry Up"
	elif current > first:
	...
	#engine.runAndWait()
	print "compare1"
def compareVelocity2(avg, std , v):
	if (avg-std) > v:
	#	engine.say("Hurry UP")
		print "Hurry Up"
	elif (avg+std) < v:
	...
	#engine.runAndWait()
	print "compare2"
def alaramDistance(dist):
	#engine.say("%f meters Left"%(a))
	#engine.runAndWait()
	print "%f meters Lefs" %(dist)
	print "alarm"
def test(args):
	c.count += 1
def test2(args):
	d.count += 1
def jogging_finish(startT, breakT, Move):
	spendT  = time.time() - startTe - breakT
	todayV = Move / spendT
	return todayV
			...
			while 1:
				try:
					...
					if str(e)=="Channel or attrib not ready":
						...
						device.connect()
					continue
			return time.time()-tempT,restartSteps-stopSteps
...
while 1:
	...
	countLoop=0
	HaveData=0
	buffer=client_socket.recv(8000)
	print buffer
	data2=buffer.split('.')
	Event=data2[0]
	userName=data2[1]
		...
		if Event=="marathon":
			...
			while True:
				...
				#	engine.runAndWait()
					...
					db.commit()
				elif int(time.time()-t)%3==0:#300==0:			#not sector.
...
#finish marathon
		...
		if Event=="jogging":
			...
			while isStart==1:
				...
				x.isr(mraa.EDGE_BOTH,test,test)
				time.sleep(3)
				print "d.count=%d"%(d.count)
				if d.count==2 or d.count==4:
					print "d,count"
					First=0
					...
					if First==0:
						...
						if d.count==2: 
							d.count=0
							compareVelocity1(currentV,firstV)
						elif d.count==4:
							d.count=0
						...
						if d.count==2: 
							...
							c.count=0
							d.count=0
				elif TargetRange <= MoveOfUser or c.count==4:
					#spendTime=time.time()-startTime-breakTime
					#todayV=MoveOfUser/spendTime
					todayV = jogging_finish(startTime, breakTime, MoveOfUser)		
					...
					a,b=func_stop(device)
					breakTime=breakTime+a
					breakSteps=breakSteps+b
				else:
					c.count=0
					d.count=0
